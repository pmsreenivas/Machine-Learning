...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        ###############################################################################################
        #raise NotImplementedError("Not Implemented function: __init__, class: linear_layer")
        self.gradient['W'] = np.full((input_D, output_D), 0)
        self.gradient['b'] = np.full((1, output_D), 0)
...
class relu:
    ...
    def forward(self, X):
        ...
        forward_output = np.maximum(X, np.zeros_like(X))
        self.mask = (forward_output > 0).astype(int)
        return forward_output
